...
Original file is located at
    https://colab.research.google.com/drive/1hBCPmJzEYUPGGnEwcLlzauvezvdajslo
...
def answer_one():
    ...
    Fdf = pd.merge(df,GDP,how='inner',left_on='Country',right_on='Country')
    Fdf = Fdf.set_index('Country')
...
def answer_three():
    Top15 = answer_one()
    GDPavg = Top15[['2006','2007','2008','2009','2010','2011','2012','2013','2014','2015']].mean(axis=1).rename('avgGDP').sort_values(ascending=False)
    return GDPavg
answer_three()
...
def answer_five():
    Top15 = answer_one()
    SPC = Top15['Energy Supply per Capita'].mean()
    return SPC
...
def answer_six():
    Top15 = answer_one()
    Max = Top15[Top15['% Renewable'] == max(Top15['% Renewable'])]
    Res=Max.index.tolist()[0],Max['% Renewable'].tolist()[0]
    return Res
...
def answer_seven():
    ...
    CR = Top15[Top15['Citation_Ratio'] == max(Top15['Citation_Ratio'])]
    Res=CR.index.tolist()[0],CR['Citation_Ratio'].tolist()[0]
    return Res 
